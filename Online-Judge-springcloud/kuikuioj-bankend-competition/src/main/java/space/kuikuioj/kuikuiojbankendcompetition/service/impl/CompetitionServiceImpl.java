...
import space.kuikuioj.kuikuiojbankendmodel.entity.*;
import space.kuikuioj.kuikuiojbankendmodel.vo.CompetitionLeaderboardVO;
import space.kuikuioj.kuikuiojbankendmodel.vo.CompetitionVO;
...
public class CompetitionServiceImpl extends ServiceImpl<CompetitionMapper, Competition> implements CompetitionService {
        ...
        try {
            ...
            for (Map.Entry<String, CompetitionSubmitRequest.QuestionSubmissionInfo> entry : questionSubmissions.entrySet()) {
                ...
                try {
                    ...
                    Long submitId = questionSubmitFeignClient.submitInner(submitReq);
                } catch (Exception e) {
                    // 继续处理下一个题目，不影响整体提交
                }
            }
    ...
    public List<CompetitionLeaderboardVO> getCompetitionLeaderboard(Long competitionId) {
        ...
        try {
            ...
            for (int i = 0; i < participants.size(); i++) {
                ...
                User user = userFeignClient.selectByIdInner(participant.getUserId());
                if (user != null) {
                ...
                Long submitCount = questionSubmitFeignClient.selectCountInner(participant.getUserId(),competitionId);
                ...
                submitQueryWrapper.eq("userId", participant.getUserId());
                submitQueryWrapper.eq("competitionId", competitionId);
                Long acceptCount = questionSubmitFeignClient.selectAcceptCount(participant.getUserId(),competitionId);
                ...
                submitQueryWrapper.eq("userId", participant.getUserId())
                        ...
                        .eq("status", 2); // 通过的提交
                List<QuestionSubmit> submits = questionSubmitFeignClient.selectList(participant.getUserId(),competitionId);
    ...
    private CompetitionVO competitionToVO(Competition competition) {
        ...
        // 设置创建人信息
        User creator = userFeignClient.selectByIdInner(competition.getCreatorId());
        if (creator != null) {
    ...
    public boolean submitPaper(Long competitionId, Long userId) {
        ...
        try {
            ...
            for (CompetitionQuestion competitionQuestion : competitionQuestions) {
                ...
                List<QuestionSubmit> submits = questionSubmitFeignClient.selectList2(userId,competitionId,questionId);
                ...
                if (!submits.isEmpty()) {
                    ...
                    if (!hasAccepted && latestSubmit != null) {
                        ...
                        questionSubmitFeignClient.updateById( latestSubmit.getId(),competitionId);
